...
<style>
    ...
    .stButton>button.fab-btn:active { transform: scale(0.9); }
...
try:
    ...
    with tab1:
        ...
        if st.session_state["show_add_form"]:
            with st.container():
                ...
                with st.form("add_new_asset"):
                    ...
                    if btn1.form_submit_button("確認新增", use_container_width=True):
                        if n_name:
                            ...
                            st.session_state["show_add_form"] = False
                            st.rerun()
                    if btn2.form_submit_button("取消", use_container_width=True):
                        st.session_state["show_add_form"] = False
                        st.rerun()
                st.markdown("---")
        ...
        for m_cat in main_cat_order:
            ...
            st.markdown(f"""
                ...
                <div style="background:#FAFAFB; padding: 10px 20px 20px 20px; border-top:1px solid #F2F2F7;">{detail_html}</div>
                </details>
            """, unsafe_allow_html=True)
    ...
    with tab2:
        ...
        if b1.button("➕ 增加持倉", use_container_width=True):
            new_q = curr_q + change
            raw_df.loc[raw_df['項目'] == target_item, '持有數量'] = new_q
            raw_df.loc[raw_df['項目'] == target_item, '總價值公式'] = new_q * u_price
